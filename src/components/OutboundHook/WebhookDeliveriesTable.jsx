...
import { useWebhookDeliveries, useRetryDelivery, useResendByDate, useProcessPending } from '../../hooks/outboundWebhookHooks';
...
const StatusBadge = ({ status }) => (
  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full capitalize ${statusColors[status?.toLowerCase()] || 'bg-gray-100 text-gray-600'}`}>
    {status}
  </span>
);
...
const WebhookDeliveriesTable = () => {
  const [filters, setFilters] = useState({ pageNumber: 1, pageSize: 20, status: '', eventType: '', configurationId: '' });
  ...
  const handleResendByDate = async () => {
    ...
    await resendByDateMutation.mutateAsync({ fromDate: resendFrom, toDate: resendTo });
      ...
      <div className="bg-white rounded-lg shadow">
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex flex-wrap gap-3 items-center justify-between">
          <div className="flex flex-wrap gap-3">
            <input
              ...
              onChange={(e) => setFilters((f) => ({ ...f, eventType: e.target.value, pageNumber: 1 }))}
            ...
            <select
              ...
              onChange={(e) => setFilters((f) => ({ ...f, status: e.target.value, pageNumber: 1 }))}
              ...
              {deliveries.map((d) => (
                <tr key={d.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="font-medium text-gray-900 text-xs">{d.webhookConfigurationName || '—'}</div>
                  ...
                  <td className="px-6 py-4 text-xs text-gray-500 whitespace-nowrap">
                    ...
                    {d.nextRetryAt && (
                      <div className="text-yellow-600">Next: {formatDate(d.nextRetryAt)}</div>
                    )}
                  </td>
                  <td className="px-6 py-4">
                    ...
                    {d.errorMessage && (
                      <div className="text-xs text-red-500 truncate max-w-xs mt-1" title={d.errorMessage}>
                        {d.errorMessage}
                      </div>
                    )}
                  </td>
                  <td className="px-6 py-4">
                    {(d.status?.toLowerCase() === 'failed' || d.status?.toLowerCase() === 'pending') && (
                      <button
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between text-sm text-gray-600">
          ...
          <div className="flex gap-2">
            ...
            <span className="px-3 py-1">Page {filters.pageNumber}</span>
            <button
      ...
      {showResendModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Resend Deliveries by Date Range</h3>
            <div className="space-y-3">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">From</label>
                <input type="datetime-local" value={resendFrom} onChange={(e) => setResendFrom(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">To</label>
                <input type="datetime-local" value={resendTo} onChange={(e) => setResendTo(e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
              </div>
            </div>
