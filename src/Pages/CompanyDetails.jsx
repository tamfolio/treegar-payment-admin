...
const CompanyDetails = () => {
  ...
  const { data: transactionsResponse, isLoading: transactionsLoading } =
    useCompanyTransactions(id, 1, 5);
  const transactions = transactionsResponse?.data?.items || [];

  const accountBalances = company?.accountBalances || [];
  ...
  if (isError || !company) {
    return (
      <Layout
        ...
        subtitle="Error loading company information"
      >
        <div className="p-8 text-center">
  ...
  const { date: createdDate, time: createdTime } = formatDate(
    company.createdAt
  );
  const { date: approvedDate, time: approvedTime } = formatDate(
    company.approvedAt
  );
                    ...
                    >
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-gray-500">
                          Ledger: ₦{account.ledgerBalance?.toLocaleString()}
                        </p>
                      </div>
                    </div>
                  ))}
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <p className="mt-1">
                  <span
                    className={`inline-flex px-2 py-1 text-sm font-semibold rounded-full ${getStatusBadge(
                      company.status
                    )}`}
                  >
                    {company.status}
              ...
              <div>
                ...
                <p className="mt-1">
                  <span
                    className={`inline-flex px-2 py-1 text-sm font-semibold rounded-full ${getApprovalBadge(
                      company.isApproved
                    )}`}
                  >
                    {company.isApproved ? "Approved" : "Pending Approval"}
              ...
              {company.createdByAdminId && (
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">
                    Admin #{company.createdByAdminId}
                  </p>
                </div>
              )}
              ...
              {company.approvedByAdminId && (
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900">
                    Admin #{company.approvedByAdminId}
                  </p>
                </div>
              )}
            </div>
